...
function renderCertifications() {
  const section = baseSection('certifications');
  const items = (state.content.certifications || []).filter((item) => item.visible !== false);
  const track = document.createElement('div');
  track.className = 'certification-marquee reveal';
  const rail = document.createElement('div');
  rail.className = 'certification-rail';
...
function renderSkills() {
  ...
  const groups = [
    { key: 'commercialSkills', label: 'Commercial skills', kicker: 'B2B KAM', className: 'skill-group-commercial', limit: 12 },
