...
palettes.glass = [
  '#FFFFFF','#F8FBFF','#F3F8FF','#EEF6FF','#EAF3FF','#E4F0FF','#DCEBFF','#D4E7FF','#CBE1FF','#C2DCFF',
  '#F7FBFF','#EFF9FF','#E8F7FF','#DFF4FF','#D6F0FF','#F9F7FF','#F4F0FF','#EEE8FF','#E8DFFF','#E2D7FF',
  '#FFF7FB','#FFF0F7','#FFE8F3','#FFE0EF','#FFD8EA','#FFF8F5','#FFF1EB','#FFEAE1','#FFE3D7','#FFDBCD',
...
const panelLabels = {
  identity: 'Identity', theme: 'Design system', sections: 'Section order', metrics: 'Impact metrics', certifications: 'Certifications',
...
const configs = {
  ...
  certifications: {
    key: 'certifications', overline: '05 · CERTIFICATIONS', title: 'Show completed credentials and learning in progress honestly.', description: 'Add, remove, and reorder up to 50 certifications. Upload a mini certificate image or PDF; clicking it on the public site opens the credential.', label: 'Certifications', singular: 'certification', titleKey: 'title',
    fields: [
      { key: 'title', label: 'Certification / course title', wide: true },
      { key: 'issuer', label: 'Issuer / provider' },
      { key: 'status', label: 'Status (Completed / In progress / Planned)' },
...
function renderSkills() {
  ...
  panel.innerHTML = panelHeading('08 · SKILLS', 'Present the complete B2B KAM toolkit.', 'Four editable groups keep the public presentation structured while allowing significantly more depth than a one-page CV.') +
    stringCollectionBlock('commercialSkills', 'Commercial skills', 'commercial skill', 'Up to 300 strategic-account, business-development, tender, pricing, solution-selling, and channel capabilities.') +
...
function renderEducation() {
  ...
  let html = panelHeading('10 · EDUCATION', 'Turn education into visible evidence.', 'Manage qualifications, optional subjects, a dedicated thesis showcase, downloadable research documents, and a separate GATE achievement card.') +
    `<article class="editor-card"><div class="editor-card-head"><h3>Education section introduction</h3></div><div class="field-grid">
      ${field('sections.education.eyebrow', 'Section label', 'text', { wide: true })}
      ${field('sections.education.title', 'Main heading', 'textarea', { wide: true })}
      ${field('sections.education.intro', 'Introductory description', 'textarea', { wide: true })}
